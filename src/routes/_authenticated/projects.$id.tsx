...
function ProjectDetailPage() {
  ...
  const { hasAdminRole, isAdmin, isAuthenticated, loading, viewMode } = useAuth();
  ...
  const mode = hasAdminRole && viewMode === "admin" ? "admin" : "client";
  const backTo = mode === "admin" ? "/admin/projects" : "/dashboard/projects";
  const topbarTitle = project?.project_name ?? "Project";
  const topbarSubtitle =
    project?.clients?.company_name ?? "Project workspace with team, invoices and communication.";

  return (
    <SidebarProvider defaultOpen={true} className="min-h-svh">
      <div className="flex min-h-svh w-full">
        {mode === "admin" ? <AdminSidebar /> : <ClientSidebar />}
        <div className="flex min-w-0 flex-1 flex-col bg-background">
          <PortalTopbar title={topbarTitle} subtitle={topbarSubtitle} mode={mode} />
          <main className="flex-1 px-4 py-6 sm:px-6 lg:px-8">
            {!project ? (
              <div className="mx-auto max-w-5xl text-muted-foreground">Loading...</div>
            ) : (
              <div className="mx-auto max-w-5xl">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2 text-xs font-semibold uppercase tracking-[0.25em] text-brand-gold-deep">
                ...
                <h1 className="mt-1 font-serif text-3xl text-brand-navy">
                ...
                {isAdmin && (
                  <div className="mt-4 flex flex-wrap items-end gap-2 rounded-lg border border-dashed p-4">
                    ...
                    <div className="min-w-[160px] flex-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setNewMember({ ...newMember, role_on_project: e.target.value })
                        }
                      />
                    </div>
                    <button
                      ...
                      className="inline-flex items-center gap-1 rounded-md bg-brand-navy px-4 py-2 text-sm text-white"
                    >
                ...
                <h2 className="mt-10 font-serif text-xl text-brand-navy">
                  Quotations &amp; invoices
                </h2>
                <div className="mt-3 divide-y rounded-lg border">
                  {invoices.length === 0 && (
